...
{
    $rows= CS50::query("SELECT * FROM jobs WHERE complete = 0 ORDER BY job_date DESC");
    $jobs = [];
    $size=sizeof($rows);
    ...
    render("header_review.php","edit_proposals_form.php",["title" => "Incomplete Jobs","jobs"=>$jobs,"users"=>$users]);
}
    ...
    render("header.php","delete_confirmed.php",["title" => "Job Deleted","users"=>$users]);
}
?>